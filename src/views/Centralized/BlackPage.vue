    ...
    <div>
        <div class="whilteBreadcrumb">
            <Breadcrumb>
                ...
                <BreadcrumbItem>黑名单信息</BreadcrumbItem>
            </Breadcrumb>
        </div>
        <div class="pagetop">
            ...
            <div class="import">
                批量导入:
                <Button class="import" @click="onImport" icon="ios-download-outline">导入黑名单</Button>
            </div>
            <Modal v-model="showImportModal" title="导入黑名单" width="300px" :footer-hide="true">
                <Upload ref="upload" multiple type="drag" :headers="headers" :data="uploadObj"
                        :action="uploadUrl+'/customer/blacklist/list'"  :on-success="onUpload" :on-error="offUpload">
                    <div style="padding: 20px 0">
                        <Icon type="ios-cloud-upload" size="52" style="color: #3399ff"></Icon>
                        <p>点击或将文件拖拽到这里上传</p>
                    </div>
                </Upload>
                <p>注：上传文件请先下载<a :href="uploadUrl+'/template/黑名单模板.xls'">模板</a>，按要求填写、上传进行导入。</p>
            </Modal>
        </div>
        <div class="page">
            <!-- 黑名单标签页 -->
            ...
            <div class="search">
                <Button class="createBtn" type="primary" @click="addCustomer">添加黑名单</Button>
            ...
            <Modal footer-hide
                ...
                <Select style="width: 350px" v-if="typeof customerIndex == 'number'" v-model="offReason" placeholder="请选择不符合授信原因......">
                ...
                <Input style="margin: 10px 0;width: 350px"  v-model="comment" v-if="offReason == '其他'" type="textarea" placeholder="请输入原因..." />
                <div style="width: 100%;text-align: right">
...
export default {
  data () {
    return {
      ...
      form: {},
      offReason: '', // 加入黑名单的原因
      comment: '', // 当加入黑名单原因位其他时  这个就是真正的原因
      customerIndex: '',
      search1: {
        idNumber: ''
      },
  ...
  methods: {
    ...
    lettersList (params = {}) {
      var vue = this
      this.$store.dispatch(actions.get.BLACK_LIST, {
    ...
    deleteUser () {
      ...
      setTimeout(() => {
        ...
        this.$store.dispatch(actions.delete.BLACK_PERSON, this.model.id).then(rep => {
    ...
    addCustomer1 () {
      ...
      if (vue.customerIndex === '') {
        vue.$Message.error('请选择您要添入黑名单的客户')
      } else {
